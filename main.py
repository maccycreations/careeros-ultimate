...
def index():
    ...
    with ui.left_drawer(value=True).classes('bg-slate-50'):
        ...
        pages = [('Dashboard', dashboard_page), ('Skills', skills_page), ('Career paths', career_page), ('AI roadmap', roadmap_page), ('Resume builder', resume_page), ('Live jobs', jobs_page), ('Application tracker', applications_page), ('AI hub', ai_hub_page), ('Profile & settings', settings_page)]
        for label, page in pages:
            ui.button(label, on_click=lambda p=page: (content.clear(), p(content))).props('flat align=left').classes('w-full')
